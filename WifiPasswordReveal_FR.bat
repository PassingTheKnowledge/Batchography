...
:main
    ...
    echo.
    echo Version pour Windows en Francais.
    echo.
...
:get-profile-key <1=profile-name> <2=out-profile-key>
    ...
    FOR /F "usebackq tokens=2 delims=:" %%a in (
        `netsh wlan show profile name^="%~1" key^=clear ^| findstr /C:"Contenu de la cl"`) DO (
        set result=%%a
        set result=!result:~1!
    )
    (
...
:get-profiles <1=result-variable>
    ...
    FOR /F "usebackq tokens=2 delims=:" %%a in (
        `netsh wlan show profiles ^| findstr /C:"Profil Tous les utilisateurs"`) DO (
        set val=%%a
        set val=!val:~1!
